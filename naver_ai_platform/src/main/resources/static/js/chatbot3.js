 ...
 */
 ...
 $(function(){
	//웰컴 메세지를 받기 위해서 message 입력받기 전에
	// 빈 값으로 서버에 전송하고 웰컴 메세지를 받기위해
	// $.ajax()를 별도의 함수로 분리해서 호출
	callAjax();	// 메세지 입력하지 않고 호출해서 빈값 전송-> open event -> 웰컴메세지 출력
	...
	//messagem입력하고 전송 버튼 눌렀을때
	$('#chatForm').on('submit', function(event){
		event.preventDefault();
		
		//메세지 입력하지 않고 [전송]버튼 누른 경우 웰컴 메세지 안나오도록 
		if($('#message').val() == ""){
			alert("질문을 입력하세요");
			return false;
		}
